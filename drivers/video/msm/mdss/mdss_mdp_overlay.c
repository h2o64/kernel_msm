...
static ssize_t lbm_show(struct device *dev,
			struct device_attribute *attr, char *buf)
{
	struct fb_info *fbi = dev_get_drvdata(dev);
	struct msm_fb_data_type *mfd = (struct msm_fb_data_type *)fbi->par;
	...
	return snprintf(buf, PAGE_SIZE, "%d\n",
			ctl->panel_data->panel_info.lbm_state);
}

static ssize_t lbm_store(struct device *dev,
			...
			const char *buf, size_t count)
{
	...
	mdss_mdp_clk_ctrl(MDP_BLOCK_POWER_ON, false);
	r = mdss_mdp_ctl_intf_event(ctl, MDSS_EVENT_ENABLE_LBM,
				(void *)(unsigned long)enable);
	if (r) {
		pr_err("Failed sending LBM command, r = %d\n", r);
		r = -EFAULT;
	} else
		pr_info("LBM state changed by sysfs, state = %d\n", enable);
...
static DEVICE_ATTR(lbm, S_IWUSR | S_IWGRP | S_IRUSR | S_IRGRP,
		lbm_show, lbm_store);

static struct attribute *lbm_attrs[] = {
	&dev_attr_lbm.attr,
	NULL,
};

static struct attribute_group lbm_attrs_group = {
	.attrs = lbm_attrs,
};
